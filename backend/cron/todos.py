...
from ..models import Felmeresek, MiniCrmAdatlapok, Systems
from ..utils.minicrm import Contact
...
class Main(MiniCRMWrapper):
    def fn(
        ...
        self.script_name = script_name + "_todo"
        self.log(
            "Felmérés feladatok készítése elindult", "INFO", script_name=script_name
        )
        ...
        adatlapok = [
            ...
            for i in (
                self.get_adatlapok(
                    StatusIdStr=status_id,
                )
                if status_id is not None
                else self.get_adatlapok()
            )
        ...
        for adatlap in adatlapok:
            if (
                ...
                if does_todo_exist
                else self.get_todos(todo_id=adatlap.Id).exists() is False
            ):
                self.log(
                ...
                if update_adatlap is not None:
                    urlap = self.minicrm_client.update_adatlap_fields(
                        id=adatlap.Id,
                ...
                contact = self.minicrm_client.contact_details(
                ...
                if custom_create_todo is not None:
                    ...
                    todo = self.minicrm_client.create_to_do(
                    ...
                    if todo.status_code == 200:
                        self.save_todo(todo_id=adatlap.Id)
                        continue
                    self.log(
    ...
    def create_beepites_todo(
        self, adatlap: MiniCrmAdatlapok, type: int, comment: str, deadline: str
    ):
        ...
        for beepito in beepitok:
            ...
            if todo.status_code == 200:
                self.save_todo(todo_id=str(adatlap.Id) + beepito)
                continue
            self.log(
                "Hiba akadt a feladat létrehozása közben",
                "ERROR",
    ...
    def beepites_comment(self, adatlap: MiniCrmAdatlapok, contact: dict):
        order_id = self.get_orders(adatlap_id=adatlap.Id).first().order_id
        address = self.minicrm_client.get_order(order_id=order_id).json()["Customer"]

        try:
            order = self.get_felmeres(id=adatlap.FelmeresLink.split("/")[-1])
    ...
    def main(self):
        models = [
            ...
            },
            {
                ...
                "type": lambda adatlap: self.get_setting(
                    label=adatlap.BejelentesTipusa, type="TodoId"
                ).value,
            },
            {
                "script_name": "pen_beepites",
                "filter_adatlapok": filter_beepites_adatlapok,
                "todo_comment": self.beepites_comment,
                "deadline_field": "DateTime1953",
                "type": lambda _: 199,
                "custom_create_todo": self.create_beepites_todo,
                "does_todo_exist": lambda adatlap: any(
                    not self.get_todos(todo_id=str(adatlap.Id) + beepito).exists()
...
def garancia_comment(adatlap: MiniCrmAdatlapok, contact: Contact):
...
def filter_beepites_adatlapok(adatlap: MiniCrmAdatlapok):
    return (
        ...
        and adatlap.StatusIdStr != "Sikeres"
    )
