...
def main():
    ...
    adatlapok = get_all_adatlap(category_id=29, criteria=lambda adatlap: adatlap["StatusId"] in [3008, 3012])
    if not adatlapok:
    ...
    for adatlap in adatlapok:
        order = Order.objects.get(webshop_id=Orders.objects.get(adatlap_id=adatlap["Id"]).order_id)
        if adatlap["StatusId"] == 3008 and order.order_status == "Completed":
            resp = update_order_status(order.webshop_id)
            if not resp.ok:
                log("Hiba történt a MiniCRM API hívás során", "ERROR", "pen_erp_status_sync", f"OrderId: {order.order_id}. Response: {resp.text}")
                continue
        if order.payment_status == "Completed":
            ...
            if not resp.ok:
                log("Hiba történt a MiniCRM API hívás során", "ERROR", "pen_erp_status_sync", f"OrderId: {order.order_id}. Response: {resp.text}")
                continue
        log("MiniCRM ERP státusz szinkron sikeresen lefutott", "INFO", "pen_erp_status_sync", adatlap["Id"])
main()